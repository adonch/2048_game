...
class Game {
    ...
    moveLeft() {
        if (this.canMove()) this.doMove("left");
    }
    moveRight() {
        if (this.canMove()) this.doMove("right");
    }
    moveUp() {
        if (this.canMove()) this.doMove("up");
    }
    moveDown() {
        if (this.canMove()) this.doMove("down");
    }
    doMove(direction) {
        const previousState = JSON.stringify(this.getState());
        let board = JSON.parse(JSON.stringify(this.getState()));
        if (direction === "up" || direction === "down") board = this.transposeBoard(board);
        if (direction === "right" || direction === "down") board = board.map((row)=>row.slice().reverse());
        for(let i = 0; i < 4; i++){
    ...
    checkGameStatus() {
        ...
        const boardFull = this.getState().every((row)=>row.every((cell)=>cell !== 0));
        if (boardFull && !this.canMove()) this.status = "lose";
    }
    updateGameStatus() {
    ...
    transposeBoard(board) {
        ...
        for(let i = 0; i < size; i++)for(let j = 0; j < size; j++)transposed[i][j] = board[j][i];
        return transposed;
    }
    restart() {
        this.initialState = [
